...
/*599	199 */
SELECT COUNT(*) AS total_customers FROM Customers WHERE JoinDate > '2020-12-31';
    /*	5*/
SELECT SUM(Quantity) AS total_books_ordered FROM Orders;
	/*	25*/
    
SELECT CustomerID , SUM(Quantity) AS total_books_ordered FROM Orders GROUP BY CustomerID;
	/* 	CustomerID	total_books_ordered
	101	3
	102	4
	103	5
	104	6
...
select BookID from Orders group by BookID order by SUM(Quantity) desc limit 1;
  /* 5*/  
select Books.BookID, SUM(Books.Price * Orders.Quantity) AS TotalRevenue from Books join Orders on Books.BookID = Orders.BookID group by Books.BookID;
/* 	BookID	TotalRevenue
	1	597
	2	1196
	3	1995
